...
각 상태는 상태별 간격으로 순환하는 여러 **프레임** (상태당 4~8개)을 가지며, 깜빡임 같은 순간적 표정에는 프레임별 duration 오버라이드가 적용됩니다:

```
프레임 1 (대기)    프레임 2 (깜빡임)  프레임 3 (대기)    프레임 4 (윙크)
   ▗▟███▙▖           ▗▟███▙▖          ▗▟███▙▖            ▗▟███▙▖
  ▐█ ◠ ◠ █▌         ▐█ ─ ─ █▌        ▐█ ◠ ◠ █▌          ▐█ ◠ ─ █▌
  ▐█▄▄▄▄▄█▌         ▐█▄▄▄▄▄█▌        ▐█▄▄▄▄▄█▌          ▐█▄▄▄▄▄█▌
   ▀█████▀           ▀█████▀          ▀█████▀             ▀█████▀
...
**상태별 애니메이션 타이밍** — 각 사용량 수준마다 고유한 리듬:

| 상태        | 간격           | 프레임 | 느낌                          |
| ----------- | -------------- | ------ | ----------------------------- |
| `relaxed`   | 4000–6000ms    | 8      | 느긋, 가끔 깜빡임과 윙크     |
| `normal`    | 3000–5000ms    | 6      | 집중, 주기적 깜빡임          |
| `concerned` | 2500–4000ms    | 5      | 불안, 시선 이동               |
| `critical`  | 1500–2500ms    | 2      | 빠른 교대, 긴박감            |
| `rateLimit` | 5000–8000ms    | 6      | 느리고 졸림                   |
| `error`     | 2000–3500ms    | 4      | 혼란, 간헐적 깜빡임          |

**프레임별 duration 오버라이드** (`frameDurations`) — 순간적 표정은 상태 간격 대신 짧은 고정 duration 사용:

| 표정         | Duration | 설명                            |
| ------------ | -------- | ------------------------------- |
| 깜빡임 (──)  | 150ms    | 자연스러운 눈 감김, 거의 순간적 |
| 윙크 (◠─)    | 200ms    | 한쪽 눈만 감는 장난스러운 표정  |
| 시선이동 (◔◔)| 300ms    | 잠깐 곁눈질                     |
| 졸림 (_ _)   | 400ms    | 무거운 눈꺼풀 (rate limited)    |

**타이밍 우선순위**: `frameDurations[i]` > `state.timing` > `preset.defaultTiming` > 생성자 기본값 (3~5초)
...
- 한 번에 하나의 메시지를 표시하며, animator의 speech 타이머가 관리
- 매 렌더가 아닌 45~75초(`speechTiming`) 간격으로 메시지 교체
- 상태 변경 시 새 상태의 메시지 풀에서 즉시 새 대사 선택
- 각 상태는 언어별 메시지 풀을 가짐 (랜덤 선택)
- Animator가 `currentMessage`를 노출하고 renderer가 사용, `--once` 모드에서는 랜덤 pick 폴백
...
- 상태별 복수 프레임 (최소 2개, 권장 4~8개로 다양한 대기 애니메이션)
- 선택적 상태별 `timing` 오버라이드 (`AnimationTiming`)
- 선택적 `frameDurations` 배열 (깜빡임, 윙크 등 프레임별 duration 오버라이드)
- 프리셋 레벨 `defaultTiming` 및 `speechTiming`
